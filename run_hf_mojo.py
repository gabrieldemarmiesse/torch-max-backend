...
Usage:
    uv run python run_hf_mojo.py            # run all models
    uv run python run_hf_mojo.py bert gpt2  # run a subset
"""
...
def _compare(model, inputs, extract):
    ...
    model.to(DEVICE)
    mojo_inputs = {k: v.to(DEVICE) for k, v in inputs.items()}
    with torch.no_grad():
        out = extract(model(**mojo_inputs)).to("cpu")
