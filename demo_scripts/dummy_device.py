...
class MyDeviceTensor(torch.Tensor):
    ...
    def __new__(cls, size, dtype, raw_data=None, requires_grad=False):
        ...
        print("Creating MyDeviceTensor with raw_data:", raw_data)
...
c = (a + b).sum()
c.backward()
assert np.allclose(a.grad.raw_data, np.ones((2, 2)))


def my_custom_backend(gm, example_inputs):
    print("running the compiled version")
    return gm.forward
...
simple_add = torch.compile(simple_add, backend=my_custom_backend)
output = simple_add(a, b)
print("output:", output)
