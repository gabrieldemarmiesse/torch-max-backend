...
from torch_mojo_backend.flags import debug_graph
from torch_mojo_backend.torch_compile_backend.utils import get_error_message

# TODO: directory creation and cleanup
output_directory = Path("/tmp/.torch_mojo_backend_debug")
            ...
            if isinstance(tensor, torch.Tensor):
                ...
                if loaded_tensor.dtype != true_tensor_from_torch.dtype:
                    raise ValueError(
                        ...
                        f"This is likely a bug in the Mojo backend. Please open an issue."
                    )
                # Check shape
