...
def _mma_tile_impl[
    ...
    var rows_in_group = min(
        Int32(_GROUP_M), blocks_m - gid * Int32(_GROUP_M)
    )
...
def _gemm_nn(
    output: _Ptr, a: _Ptr, b: _Ptr, bias: _Ptr, m: Int, n: Int, k: Int,
    has_bias: Int, a_fast: Int, b_fast: Int, c_pair: Int, batch_count: Int,
):
    _mma_tile_impl[False, False, 128, 128, False, True](
        output, a, b, bias, m, n, k, 0, 0, 0, has_bias, a_fast, b_fast,
        c_pair, batch_count,
    )
...
def _bmm_nn(
    output: _Ptr, a: _Ptr, b: _Ptr, m: Int, n: Int, k: Int,
    c_bstride: Int, a_bstride: Int, b_bstride: Int,
    a_fast: Int, b_fast: Int, c_pair: Int, batch_count: Int,
):
    _mma_tile_impl[False, False, 128, 128, False, True](
        output, a, b, a, m, n, k, c_bstride, a_bstride, b_bstride, 0,
        a_fast, b_fast, c_pair, batch_count,
    )
...
@always_inline
def _mma_tile_wide[TA: Bool, TB: Bool](
    output: _Ptr,
    a: _Ptr,
    b: _Ptr,
    bias: _Ptr,
    ...
    if transpose_a:
        if transpose_b:
            ctx.enqueue_function[_gemm_tt_wide](
                output, a, b, bias, m, n, k, hb, a_fast, b_fast, c_pair, 1,
                grid_dim=(grid_x,), block_dim=(_THREADS,),
            )
        else:
    ...
    if transpose_a:
        if transpose_b:
            ctx.enqueue_function[_bmm_tt_wide](
                output, a, b, m, n, k,
                c_bstride, a_bstride, b_bstride,
                a_fast, b_fast, c_pair, batch_count,
                grid_dim=(grid_x, 1, grid_z), block_dim=(_THREADS,),
            )
        else:
...
def enqueue_bf16_gemm(
    ...
    if grid_x == 0:
        _enqueue_gemm_wide(
            output, a, b, bias, m, n, k, hb, a_fast, b_fast, c_pair,
            transpose_a, transpose_b, ctx,
        )
        return
            ...
            if transpose_a:
                if transpose_b:
                    ctx.enqueue_function[_gemm_tt_w64_f](
                        output, a, b, bias, m, n, k, hb,
                        a_fast, b_fast, c_pair, 1,
                        grid_dim=(grid_x,), block_dim=(_THREADS,),
                    )
                else:
...
def enqueue_bf16_bmm(
    ...
    if not opt_ok:
        _enqueue_bmm_wide(
            output, a, b, batch_count, m, n, k,
            output_batch_stride, a_batch_stride, b_batch_stride,
            a_fast, b_fast, c_pair, transpose_a, transpose_b, ctx,
        )
        return
    var fastk = _fast_proof(
        output, a, b, m, n, k,
