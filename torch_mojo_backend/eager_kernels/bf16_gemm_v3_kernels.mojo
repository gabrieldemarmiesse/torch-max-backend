...
@__llvm_arg_metadata(a_tma, `nvvm.grid_constant`)
@__llvm_arg_metadata(b_tma, `nvvm.grid_constant`)
@__name("nanogpt_bf16_gemm_v3_nt_wgmma_tma_s2")
def _v3_nt_wgmma_tma_s2(
    a_tma: _V3_A_TMA,
    b_tma: _V3_B_K_TMA,
    output: _V3_PTR,
    m: Int,
    ...
    comptime if _is_sm_9x():
        ...
        if thread_idx.x == 0:
            mbar[0].expect_bytes(Int32(TMA_BYTES))
            a_tma.async_copy(a_smem0, mbar[0], (0, m0))
            b_tma.async_copy(b_smem0, mbar[0], (0, n0))
            if num_tiles > 1:
                mbar[1].expect_bytes(Int32(TMA_BYTES))
                a_tma.async_copy(a_smem1, mbar[1], (_V3_BK, m0))
                b_tma.async_copy(b_smem1, mbar[1], (_V3_BK, n0))
        barrier()
        ...
        while tile < num_tiles:
            ...
            if future < num_tiles and thread_idx.x == 0:
                ...
                if stage == 0:
                    mbar[0].expect_bytes(Int32(TMA_BYTES))
                    a_tma.async_copy(a_smem0, mbar[0], (future_k, m0))
                    b_tma.async_copy(b_smem0, mbar[0], (future_k, n0))
                else:
                    mbar[1].expect_bytes(Int32(TMA_BYTES))
                    a_tma.async_copy(a_smem1, mbar[1], (future_k, m0))
                    b_tma.async_copy(b_smem1, mbar[1], (future_k, n0))
            tile += 1
...
def _v3_enqueue_nt_wgmma_tma_s2(
    output: _V3_PTR,
    a: _V3_PTR,
    b: _V3_PTR,
    m: Int,
    ...
    var b_desc = create_tma_descriptor[_V3_BF16, 2, _V3_SWIZZLE](
        ...
        IndexList[2](n, k),
        IndexList[2](k, 1),
        IndexList[2](_V3_BN, _V3_BK),
    )
    var a_tma = _V3_A_TMA(a_desc)
    var b_tma = _V3_B_K_TMA(b_desc)
    ctx.enqueue_function[_v3_nt_wgmma_tma_s2](
        a_tma,
        b_tma,
        output,
        m,
